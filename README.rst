...
We'd like the community's `help <https://openai.com/blog/jiminy/#help>`_
...
Install Universe
~~~~~~~~~~~~~~~~
To get started, first install ``jiminy``:

.. code:: shell

    git clone https://github.com/openai/jiminy.git
    cd jiminy
    pip install -e .
...
To get started, clone the ``jiminy`` repo:
...
Build a docker image, tag it as 'jiminy':

.. code:: shell

    docker build -t jiminy .
...
.. code:: shell
    ...
    docker run --privileged --rm -e DOCKER_NET_HOST=172.17.0.1 -v /var/run/docker.sock:/var/run/docker.sock jiminy pytest

Here's a breakdown of that command:

* ``docker run`` - launch a docker container
* ``--rm`` - delete the container once the launched process finishes
* ``-e DOCKER_NET_HOST=172.17.0.1`` - tells the jiminy remote (when launched) to make its VNC connection back to this docker-allocated IP
...
* ``jiminy`` - use the imaged named 'jiminy' built above
...
To do some actual development work, you probably want to do another volume map from the jiminy repo on your host into the container, then shell in interactively:

.. code:: shell

    docker run --privileged --rm -it -e DOCKER_NET_HOST=172.17.0.1 -v /var/run/docker.sock:/var/run/docker.sock -v (full path to cloned repo above):/usr/local/jiminy jiminy python
...
* When installing ``jiminy``, you may see ``warning`` messages.  These lines occur when installing numpy and are normal.
...
* While we don't officially support Windows, we expect our code to be very close to working there. We'd be happy to take pull requests that take our Windows compatibility to 100%. In the meantime, the easiest way for Windows users to run jiminy is to use the alternate configuration described above.
...
* The **client** is a `VNCEnv
  <https://github.com/openai/jiminy/blob/master/jiminy/envs/vnc_env.py>`_
...
Now that you've installed the ``jiminy`` library, you should make
...
.. code:: python
  ...
  import gym
  import jiminy  # register the jiminy environments
...
automatically pull the ``quay.io/openai/jiminy.flashgames`` image,
...
.. image:: https://github.com/openai/jiminy/blob/master/doc/dusk-drive.png?raw=true
     :width: 600px
...
* First, we import the `gym <https://github.com/openai/gym>`__ library,
  ...
  ``jiminy``, which `registers
  <https://github.com/openai/jiminy/blob/master/jiminy/__init__.py>`__
...
* Next, we create the environment instance. Behind the scenes, ``gym``
  ...
  <https://github.com/openai/jiminy/blob/master/jiminy/envs/vnc_env.py#L88>`__
  object which has been `wrapped
  <https://github.com/openai/jiminy/blob/master/jiminy/wrappers/__init__.py#L42>`__
...
* The call to ``configure()`` connects the client to a remote
  ...
  page on `jiminy internal communication protocols
...
* We call ``step`` in what looks like a busy loop. In reality, there
  is a `Throttle
  <https://github.com/openai/jiminy/blob/master/jiminy/wrappers/__init__.py#L18>`__
  ...
  ``step`` will `sleep
  <https://github.com/openai/jiminy/blob/master/jiminy/wrappers/throttle.py>`__
...
to common problems
<https://github.com/openai/jiminy/wiki/Solutions-to-common-problems>`__ -
and add to it if your solution isn't there!

You can also search through the `issues
<https://github.com/openai/jiminy/issues?utf8=%E2%9C%93&q=is%3Aissue>`__
...
* Get started training RL algorithms! You can try out the `Universe Starter Agent <https://github.com/openai/jiminy-starter-agent>`_, an implementation of the `A3C algorithm <https://arxiv.org/abs/1602.01783>`_ that can solve several VNC environments.
