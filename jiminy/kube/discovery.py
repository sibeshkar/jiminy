...
class VNCEnvDiscovery(object):
    ...
    def discover_batches(self):
        pods = check_with_output(self.kubectl + ['get', 'pods', '-o', 'json', '-l', 'type=jiminy'])
    ...
    def discover(self, batch, force_ready=False):
        pods = check_with_output(self.kubectl + ['get', 'pods', '-o', 'json', '-l', 'type=jiminy', '-l', 'batch={}'.format(batch)])
        pods = json.loads(pods)
