...
class SeleniumWoBEnv(DummyVNCEnv):
    ...
    def configure(self, remotes=None,
        ...
        self.web_driver_list = [WebLoader.WebLoader("Firefox") for _ in range(self.n)]
    ...
    def _step(self, action_n):
        ...
        assert [self.action_space.contains(action) for action in action_n].all(), "Expected VNCActions by received {}".format(action_n)
    ...
    def _action_impl(self, web_driver, action):
        webaction = webactionChains(web_driver.driver)
        ...
        if isinstance(action, vnc_event.PointerEvent):
            ...
            webaction.move_by_offset(webaction.x, webaction.y)
