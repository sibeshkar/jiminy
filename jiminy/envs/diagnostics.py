    ...
    def add_metadata(self, observation, info, available_at=None):
        ...
        if metadata is not None:
            ...
            if probe_received_at == 0 or self.disable_action_probes:
                ...
                logger.info('[%s] Probe is marked as received at %s, but probe_sent_at is None. This is surprising. (HINT: do you have multiple jiminy instances talking to the same environment?)', self.label, probe_received_at)
...
#         """TODO: This needs be moved to jiminy-envs and run there. Otherwise it only works if the env and agent
