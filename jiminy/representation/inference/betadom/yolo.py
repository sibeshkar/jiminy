...
class YoloModel():
    ...
    def create_model(self, load_pretrained=True, freeze_body=2,
            weights_path="/home/prannayk/keras-yolo3/model_data/darknet53_weights.h5"):
        image_input = Input(shape=(None, None, 3), name="yolo/input_image")
        height, width = self.input_shape
        num_anchors = len(self.anchors)

        y_true = [Input(shape=(height//{0:32, 1:16, 2:8}[i], width//{0:32, 1:16, 2:8}[i], \
                num_anchors//3, self.num_classes+5)) for i in range(3)]
        ...
        if load_pretrained:
            ...
            if freeze_body in [1, 2]:
                self.freeze_model_weights(model=model_body, freeze_body=freeze_body)
    ...
    def train_stage1(self, learning_rate, batch_size=32, epochs=50, callbacks=None):
        self.freeze_model_weights(self.model_body, 2)
        self.model.compile(optimizer=Adam(learning_rate=learning_rate),
                loss={'yolo_loss': lambda y_true, y_pred: y_pred})
        self.fit(batch_size, epochs, callbacks=callbacks)

    def fit(self, batch_size=32, epochs=50, initial_epoch=0, callbacks=None):
        if callbacks is None:
            logging = TensorBoard(log_dir=self.log_dir, update_freq='batch')
        ...
        if not hasattr(self, "data_generator") or not hasattr(self, "validation_generator"):
            self.load_data(batch_size=batch_size)

        self.model.fit_generator(self.train_data_generator,
                steps_per_epoch=max(1, self.n_train//batch_size),
                validation_data=self.validation_data_generator,
                validation_steps=max(1, self.n_val//batch_size),
                epochs=epochs,
                max_queue_size=150,
    ...
    def load_data(self, batch_size=32):
        ...
        self.n_val = int(len(data) * self.validation_split)
        self.n_train = len(data) - self.n_val
        train_data = data[:self.n_train]
        validation_data = data[self.n_train:]

        print('Train on {} samples, val on {} samples, with batch size {}.'.format(self.n_train,
            self.n_val, batch_size))
        self.train_data_generator = self.data_generator_wrapper(train_data, batch_size=batch_size)
        self.validation_data_generator = self.data_generator_wrapper(validation_data, batch_size=batch_size)
    ...
    def data_generator(self, data, batch_size):
        ...
        while True:
            ...
            for b in range(batch_size):
                if i==0:
                    np.random.shuffle(data)
                image, box = get_random_data(data[i], self.input_shape, random=True)
    ...
    def data_generator_wrapper(self, data, batch_size):
        n = len(data)
