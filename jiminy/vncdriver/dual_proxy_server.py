...
from jiminy.vncdriver.vnc_proxy_server import VNCProxyServer
from jiminy.rewarder.reward_proxy_server import RewardProxyServer
...
class DualProxyServer(VNCProxyServer):
    ...
    def close(self):
        ...
        if self.log_manager:
            os.system('/app/jiminy/bin/upload_directory.sh demonstrator_%(recorder_id)s %(directory)s %(bucket)s' %
